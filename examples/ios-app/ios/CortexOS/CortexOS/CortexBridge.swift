...
class CortexBridge {
    ...
    func startHeartbeatAgent(name: String, interval: UInt) -> String {
    ...
    // MARK: - Discovery (Multi-Protocol: Multicast + Broadcast + mDNS)
    
    func startDiscovery() -> String {
        guard let ptr = cortex_start_discovery() else { return "" }
        let result = String(cString: ptr)
        cortex_free_string(ptr)
        return result
    }
    ...
    func getPeers() -> [[String: Any]] {
        guard let ptr = cortex_get_peers() else { return [] }
        let json = String(cString: ptr)
        cortex_free_string(ptr)
